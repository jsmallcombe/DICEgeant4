	...
	fStepperCmd(0)
{
	...
	fStepperCmd = new G4UIcmdWithABool("/DetSys/phys/SetStepper",this);
	fStepperCmd->SetGuidance("Choose to invoke electron stepper");
	fStepperCmd->SetDefaultValue(false);
		...
		G4String newValue)
{
	...
	{
		fPPhysicsList->SetStepper(fStepperCmd->GetNewBoolValue(newValue));
	}
}
